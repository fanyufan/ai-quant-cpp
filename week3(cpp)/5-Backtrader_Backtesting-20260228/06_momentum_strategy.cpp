...
class MomentumStrategy : public quant::bt::Strategy {
public:
    void init(const std::vector<quant::bt::Bar>& bars) override {
        ...
        roc_ = quant::ind::roc(close, period_);
    }

    void next(size_t idx, const std::vector<quant::bt::Bar>& bars, quant::bt::Broker& broker) override {
        if (idx == 0 || idx >= roc_.size()) return;
        if (!std::isfinite(roc_[idx - 1]) || !std::isfinite(roc_[idx])) return;

        bool buy_signal  = roc_[idx - 1] <= buy_threshold_ && roc_[idx] > buy_threshold_;
        bool sell_signal = roc_[idx - 1] >= sell_threshold_ && roc_[idx] < sell_threshold_;
...
private:
    static constexpr size_t period_ = 10;
    static constexpr double buy_threshold_ = 5.0;
    static constexpr double sell_threshold_ = -5.0;
    std::vector<double> roc_;
};
...
int main(int argc, char* argv[]) {
    ...
    const std::string prefix = fmt::format("{}/06_momentum_strategy", args.output_dir);
