...
class BollingerStrategy : public quant::bt::Strategy {
public:
    ...
    void next(size_t idx, const std::vector<quant::bt::Bar>& bars, quant::bt::Broker& broker) override {
        ...
        if (!std::isfinite(bb_.upper[idx - 1]) || !std::isfinite(bb_.lower[idx - 1]) ||
            !std::isfinite(bb_.upper[idx]) || !std::isfinite(bb_.lower[idx])) {
            return;
        }
        ...
        bool sell_signal = prev_close >= bb_.upper[idx - 1] && curr_close < bb_.upper[idx];
...
int main(int argc, char* argv[]) {
    ...
    const std::string prefix = fmt::format("{}/04_bollinger_strategy", args.output_dir);
