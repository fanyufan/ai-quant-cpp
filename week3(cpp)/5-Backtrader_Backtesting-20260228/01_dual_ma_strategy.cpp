...
class DualMaStrategy : public quant::bt::Strategy {
public:
    void init(const std::vector<quant::bt::Bar>& bars) override {
        ...
        fast_ = quant::ind::sma(close, fast_period_);
        slow_ = quant::ind::sma(close, slow_period_);
    }

    void next(size_t idx, const std::vector<quant::bt::Bar>& bars, quant::bt::Broker& broker) override {
        if (idx == 0 || idx >= fast_.size()) return;
        if (!std::isfinite(fast_[idx - 1]) || !std::isfinite(slow_[idx - 1]) ||
            !std::isfinite(fast_[idx]) || !std::isfinite(slow_[idx])) {
            return;
        }

        bool golden_cross = fast_[idx - 1] <= slow_[idx - 1] && fast_[idx] > slow_[idx];
        bool death_cross  = fast_[idx - 1] >= slow_[idx - 1] && fast_[idx] < slow_[idx];
...
private:
    static constexpr size_t fast_period_ = 10;
    static constexpr size_t slow_period_ = 30;
    std::vector<double> fast_;
...
int main(int argc, char* argv[]) {
    ...
    const std::string prefix = fmt::format("{}/01_dual_ma_strategy", args.output_dir);
