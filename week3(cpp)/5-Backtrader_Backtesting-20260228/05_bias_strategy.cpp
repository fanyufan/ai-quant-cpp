...
class BiasStrategy : public quant::bt::Strategy {
public:
    void init(const std::vector<quant::bt::Bar>& bars) override {
        ...
        bias_ = quant::ind::bias(close, period_);
    }

    void next(size_t idx, const std::vector<quant::bt::Bar>& bars, quant::bt::Broker& broker) override {
        if (idx == 0 || idx >= bias_.size()) return;
        if (!std::isfinite(bias_[idx - 1]) || !std::isfinite(bias_[idx])) return;

        bool buy_signal  = bias_[idx - 1] >= buy_threshold_ && bias_[idx] < buy_threshold_;
        bool sell_signal = bias_[idx - 1] <= sell_threshold_ && bias_[idx] > sell_threshold_;
...
private:
    ...
    static constexpr double buy_threshold_ = -0.06;
    static constexpr double sell_threshold_ = 0.03;
...
int main(int argc, char* argv[]) {
    ...
    const std::string prefix = fmt::format("{}/05_bias_strategy", args.output_dir);
